...
fourth_model_clean <- fourth_model %>%
  ...
  select(-yardline_100, -ydstogo, -roof, -qtr, -half_seconds_remaining, -surface)
...
fourth_model_df <- fourth_all %>%
  ...
  mutate(
    close_game = as.integer(abs(score_differential) <= 7),
    short_to_go = as.integer(ydstogo <= 3),
    must_go = as.integer(
      qtr == 4 &
...
fourth_model_df %>%
  count(short_to_go, decision) %>%
  group_by(short_to_go) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup()
```
