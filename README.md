...
We initially did our work in 'code.Rmd', 'code2_Nat.Rmd', 'predictions.Rmd', 'predictions2.rmd', and several other files that now reside in the folder 'oldfiles'. We copy and pasted that code into slides.Rmd and paper.Rmd as they were needed. All necessary code now resides in both slides.Rmd and paper.Rmd. You can look at the other files if you're curious, but some of them are kind of a mess, since for some reason some of us added new files to the repo instead of just editing the files that were already there, but it's whatever.
